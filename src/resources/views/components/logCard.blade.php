 <div class="card shadow-lg p-4 bg-{{$color}}-800/20 rounded-xl border-0 border-{{$color}}-500">
     <div class="flex justify-between">
         <div class=" ">
             <div class="flex w-fit shadow-sm items-center {{$log->level == "EMERGENCY" ? 'animate-pulse' : ''}} gap-x-1.5 bg-{{$color}}-500 text-xs p-1 px-1.5 rounded-full font-bold text-white">

                 @if($log->level == 'NOTICE')
                 <svg class="w-[16px] h-[16px] inline" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                 ...
                 @elseif($log->level == 'ERROR' || $log->level == 'EMERGENCY'|| $log->level == 'CRITICAL')
                 <svg class="w-[16px] h-[16px] inline" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
             ...
             <div class="mt-2 font-semibold text-{{$color}}-500">{{$log->type}}</div>
         </div>
